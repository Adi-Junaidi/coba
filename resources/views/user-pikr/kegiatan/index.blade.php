...
@section('content')
  <section>
    <h1 class="d-sm-block d-none mb-3">Data Pelaporan PIK-R</h1>
    <h3 class="d-sm-none d-block mb-3">Data Pelaporan PIK-R</h3>
    <div class="row align-items-center my-4">
      <button class="btn btn-primary me-3 col-md-2" data-bs-toggle="modal" data-bs-target="#addModal">Tambah Kegiatan</button>
    </div>
    <div class="row mb-5 bg-white p-4 shadow-sm">
      @if (!$laporan_s)
        <p>Tidak ada data yang tersimpan</p>
      @else
        <div class="table-responsive" style="max-height: 400px">
          <table class="table-lg table" id="laporan-table">
            <thead>
              <tr>
                <th>No</th>
            ...
            <tbody>
              ...
              @foreach ($laporan_s as $laporan)
                <tr>
                  ...
                  <td><span class="badge bg-light-{{ $status[$laporan->status] }}">{{ $laporan->status }}</span>
                  </td>
                  <td>
                    @if ($laporan->status == 'Not Submited')
                      <a class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="top" href="/up/kegiatan/{{ $laporan->id }}" title="Tambah Kegiatan">
                        <i class="bi bi-plus-circle"></i>

                      </a>
                      <button class="btn btn-success btn-sm btn_submit" data-bs-toggle="modal" data-bs-target="#submitModal" data-id="{{ $laporan->id }}" data-toggle="tooltip" data-placement="top" title="Final Submit">
                        <i class="bi bi-fast-forward"></i>
                      </button>
                    @else
                      <a class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="top" href="/up/kegiatan/detail/{{ $laporan->id }}" title="Lihat Kegiatan">
                        <i class="bi bi-eye"></i>
                      </a>
                      @if ($laporan->status == 'Submited')
                        <a class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" href="/up/kegiatan/cancel/{{ $laporan->id }}" title="Batalkan">
...
@push('modal')
  <div class="modal fade text-left" id="addModal" aria-labelledby="" aria-hidden="true" tabindex="-1" style="display: none;">
      ...
      <div class="modal-content">
        ...
        <form action="/up/kegiatan" method="post">
          ...
          <div class="modal-footer">
            <button class="btn btn-primary btn-submit" type="submit" hidden>Tambah</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <div class="modal fade text-left" id="submitModal" aria-labelledby="" aria-hidden="true" tabindex="-1" style="display: none;">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header bg-success">
          <h5 class="modal-title white" id="myModalLabel1">
            Final Submit Data Pelaporan
          </h5>
          <button class="close rounded-pill" data-bs-dismiss="modal" type="button" aria-label="Close">
            <svg class="feather feather-x" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="18" y1="6" x2="6" y2="18"></line>
          ...
          <div class="modal-body">
            ...
            <div class="row">
              <div class="form-group col-sm-6">
                ...
                <input class="form-control" id="tempat" type="text" value="{{ $pikr->desa->kecamatan->nama }}" readonly>
              </div>

              <div class="form-group col-sm-6">
                <label for="tanggal">Tanggal</label>
                <input class="form-control" id="tanggal" type="text" value="{{ date('d/m/Y') }}" disabled readonly>
              </div>
            </div>

            <div class="form-group">
              <label for="pembina">Pembina Kelompok</label>
              <input class="form-control" id="pembina" type="text" value="{{ $pikr->pembina->nama }}" readonly>
            </div>

            <div class="form-group">
              <label for="ketua">Ketua Kelompok</label>
              <input class="form-control" id="ketua" type="text" value="{{ $ketua_info ? $ketua_info->nama : 'Ketua Kelompok Belum Di Set' }}" readonly>
            </div>
