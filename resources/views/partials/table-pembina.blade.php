@foreach ($pembina as $p)
  <tr>
    ...
    <td>
      ...
      <button class="btn btn-warning btn-sm btnUpdate" data-bs-toggle="modal" data-bs-placement="bottom" data-bs-target="#modalUpdate" data-jabatan_id="{{ $p->jabatan->id }}" data-noreg="{{ $p->no_register }}" data-nourut="{{ $p->no_urut }}" data-nama="{{ $p->nama }}" data-provinsi="{{ $p->desa->kecamatan->kabkota->provinsi->kode . ' | ' . $p->desa->kecamatan->kabkota->provinsi->nama }}" data-kabkota="{{ $p->desa->kecamatan->kabkota->kode . ' | ' . $p->desa->kecamatan->kabkota->nama }}" data-kecamatan="{{ $p->desa->kecamatan->kode . ' | ' . $p->desa->kecamatan->nama }}" data-desakel="{{ $p->desa->kode . ' | ' . $p->desa->nama }}" data-jabatan="{{ $p->jabatan->nama }}" type="button" title="Edit">
        <span class="fa-fw select-all fas"></span>
      </button>
