...
$(function () {
  $("#kabkota").change(function () {
    $.ajax({
      url: "/kecamatan/getData/" + $(this).val(),
      method: "GET",
      success: function (response) {
        ...
        $("#kecamatan").empty();
        const optionKecamatan = document.createElement("option");
        optionKecamatan.text = 'Pilih Kecamatan';
        optionKecamatan.setAttribute('hidden', true)
        kecamatan.appendChild(optionKecamatan);
        
        $("#desa").empty();
        const optionDesa = document.createElement("option");
        optionDesa.text = 'Pilih Desa ';
        optionDesa.setAttribute('hidden', true)
        desa.appendChild(optionDesa);

        response.forEach((element) => {
          const optionElement = document.createElement("option");
          optionElement.value = element.id;
          optionElement.text = element.nama;
          kecamatan.appendChild(optionElement);
        });
      },
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        response.forEach((element) => {
          ...
          optionElement.value = element.id;
          optionElement.text = element.nama;
          desa.appendChild(optionElement);
        });
      },
      error: function (xhr, status, error) {
        alert(error);
  ...
  $("#desa").change(function () {
    const tablePembina = $('#tablePembina tbody')
    tablePembina.empty()

    $.ajax({
      url: "/desa/getPikr/" + $(this).val(),
      method: "GET",
      success: function (response) {
        $.each(response, function(key, value){
          const table = document.querySelector('#tablePembina tbody')
          const tr = document.createElement('tr')

          const no = document.createElement('td')
          no.textContent = key + 1
          
          const no_regis = document.createElement('td')
          no_regis.textContent = value.no_register
          
          const nama = document.createElement('td')
          nama.textContent = value.nama

          tr.appendChild(no)
          tr.appendChild(no_regis)
          tr.appendChild(nama)
